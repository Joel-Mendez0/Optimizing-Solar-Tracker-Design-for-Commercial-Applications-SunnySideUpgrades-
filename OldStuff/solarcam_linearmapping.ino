...
void updateServoAngles(int x, int y) {
    ...
    int offsetY = centerY - y; // Inverting Y as the screen's 0 starts from the top

    // Map the screen offsets to servo angles
    // Assuming that the maximum offset will map to the full range of the servo
    // You may need to adjust the mapping factor based on your specific setup
    int servoXChange = map(offsetX, -centerX, centerX, -45, 45); // Maps the offset to a change in the servo angle
    int servoYChange = map(offsetY, -centerY, centerY, -45, 45); // Maps the offset to a change in the servo angle

    // Apply the change to the servo positions, ensuring they remain within bounds
    servoX = constrain(servoX - servoXChange, 90, 180);
    servoY = constrain(servoY + servoYChange, 90, 180);
